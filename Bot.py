...
async def kick(ctx, user: discord.User, *, motivo: str = None):
	if not motivo:
		...
		embed.add_field(name='Motivo', value=motivo)
		embed.set_footer(text='Comando realizado por {} | Open Night Oficial'.format(ctx.message.author.name))
		await bot.say(embed=embed)
...
async def clear(ctx, limit: int=None):
	if not limit:
		return await bot.say('{} Você não definiu a quantia de mensagens para ser apagar'.format(ctx.message.author.mention))
	else:
